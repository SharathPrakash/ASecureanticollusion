                                ...
                                String getCipher=null;
                                    ...
                                    if (rs.next()) {
                                        if (request.getParameter("keys").equals(rs.getString("key_"))) {
                                            getCipher = rs.getString("file_data");
                                            
                                            String screskey=rs.getString("s_key");
                                            if(screskey=="")
                                            {
                                                System.out.println("Eswar==1====Inside Triple Des"+screskey);
                                               plainText = new TrippleDes().decrypt(getCipher);
                                            }
                                            else
                                             {
                                                EncryptRsa e = new EncryptRsa();
                                                System.out.println("Eswar==1====Inside Rsa"+screskey);
                                                plainText =  e.dencrypt(session.getAttribute("Username").toString(), screskey , getCipher);
                                            }
                                %>
                                <form action="updateFile" method="post">
                                    <textarea name="gets" style="width: 500px;height: 500px;"> <%=plainText%></textarea>
                                    <input type="hidden" name="skey" value="<%=screskey%>"/>
